...
### Navigation
- **Running head:** app name at left in medium verdigris at the large title size, sentence case, not underlined; it links home, and the mark stands beside it at 1.4em of the name, so the two scale together. The name is a step up the scale from the routes across from it: the head is signed, not listed. Nav links at right in plain pencil at body size, 1rem apart, 2.75rem targets: Home, Session, History, Capture, Shelves, Plan, Review, Finished, Record. The words sat 1.5rem apart before they had marks; a mark binds to the word after it (0.25rem from it, 1rem from the word before), so each pair reads as one unit on less air than the bare words needed. Each carries one mark before its name: Phosphor Light at 1.3em, drawn in soft ink at rest — a step brighter than the pencil label, because a stroke of about 1px reads lighter than the word beside it — and inking to white with the label on hover and on the current page. The marks are decorative (`aria-hidden`, `focusable="false"`); the name is what the link is read as, and it is never dropped. They are inlined as one sprite in the layout, so a page makes no extra request and nothing is fetched from outside the binary. Name and nav share one line down to about 1050px; below that the nav drops beneath the name rather than overflowing, and it wraps rather than scrolls, which at phone width is three lines. Stats opens from History and the Record, Settings from the plan's heading. Hover inks to white; the current page is white with a verdigris underline. A rule that fades at both ends closes the head; on a page with a plate column it spans the whole composition.
...
- **Shelves index:** shelf names in their own order, as entry titles, one per line, each linking to its shelf (the name takes the entry title's verdigris underline on hover), with ↑ ↓, Rename and, only on a shelf with nothing filed on it, Delete… beneath each, pulled up half a step so the controls sit under the name they belong to. Each shelf's place in the order is written on the margin rule beside it, in pencil tabular figures with a stock knockout: the same figure as an unclaimed rank slot, because a place is where a thing sits and not a claim about it. A move answers in the status line with where the shelf landed ("Mathematics is now 2nd.") and inks that line in; the row travels with its own id, so the focus stays with the shelf and the same arrow moves it again. Rename opens the name in place as a field at the title size, with "Its tags move with it." in its hint slot, a secondary Rename and a quiet Cancel. One pencil line under the list says when a shelf can be deleted. No counts and no contents: it is a door, not a summary of the library (spec §0).
...
**The Marks Stay in the Head Rule.** The route marks are the only drawn icons in the app. Inside a page, status is a plain pencil word, a control is its verb, and a glyph is written rather than drawn (the shelf's ↑ ↓). Do not put a mark on a button, a field, an entry, a section title or a status line.
...
- **Stall:** an 0.5rem ochre point on the margin rule with a 3px stock knockout ring. An item being read that has stalled carries both marks, and the point steps below the track rather than sitting on the inked length it would hide.
...
- **Day strip:** seven columns, one per day of the week in week order: day name, a 0.75rem-wide rounded column filled to the share of the target read (verdigris, rubric when a closed day fell short), the time read and the target. Rest days are a dashed outline; today is outlined in verdigris. Each day explains itself in a small note above its column on hover, focus or tap: lifted stock, faint edge, 8px corners, the float shadow, the date in medium white, then plain lines (what was read against the target, how short it fell or that it was met, what was owed once it closed), short and owed in rubric, met in verdigris. The first and last two days align the note to their own edge so it stays on the page; it fades and settles 4px over 180ms.
...
- **Don't** draw an icon anywhere but the running head's routes, and never let a mark stand alone in place of its name.
